...
| 期待される応答 |  – 必要なすべてのノードを移動できる場合は、合格したチェックを受け取ります。 <br>- ノードがターゲットの場所に存在する場合、関連するエラーが表示されます。 リポジトリ \（delete node /var/dxml\）をクリーンアップし、アップグレードパッケージを再インストールしてから、このエンドポイントを再度トリガーします。 <br>**注意：**&#x200B;これは、3.x Experience Manager Guidesではターゲットの場所が以前に使用されていないため、一般的なエラーではありません。<br> – このスクリプトが成功しない場合は、続行せずにカスタマーサクセス チームに報告してください。 |
...
| 期待される応答 |  – 移行が成功したら、XML Documentation ソリューション バージョン 4.0をインストールできます。<br> – エラーが発生した場合は、最後のチェックポイントに復元し、エラーログとAPI出力をカスタマーサクセス チームと共有します。 |
...
2. すべての翻訳タスクを閉じました。
3. `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`のログレベルを`INFO`に設定し、新しいログファイル （例：`logs/translation_upgrade.log`）にログを記録します。
...
1. [Adobe Software Distribution Portal](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)から&#x200B;**4.2** パッケージをダウンロードします。
...
>[!TIP]
>
> バージョン **4.2.1**&#x200B;の上に&#x200B;**ホットフィックス4.2.1.3**&#x200B;をインストールすることをお勧めします。
...
2. すべての翻訳タスクを閉じます。
3. `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`のログレベルを`INFO`に設定し、新しいファイル （例：`logs/translation_upgrade.log`）にログを記録します。
...
1. パッケージのインストール後、ブラウザーキャッシュをクリアします。
1. フォルダープロファイルの「**XML Editor Configuration**」タブから`ui_config.json` ファイルをアップグレードします。
...
バージョン **4.3.1**&#x200B;を使用している場合は、直接&#x200B;**4.3.1.5**&#x200B;にアップグレードできます。

### バージョン 4.3.1.5をインストール

1. [Adobe Software Distribution Portal](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)から&#x200B;**4.3.1.5** パッケージをダウンロードします。
...
**4.3.1**、**4.3.0**、**4.2**&#x200B;または&#x200B;**4.2.1 （ホットフィックス 4.2.1.3）**&#x200B;を使用している場合は、直接&#x200B;**4.4.0**&#x200B;にアップグレードできます。
...
1. 条件`"jcr:content/jcr:mimeType!=video"`の&#x200B;**DAM アセット更新ワークフロー**&#x200B;の「*Node Created*」のランチャー – 「Globbing」の値は次のようにする必要があります。
   ...
   - 条件「`jcr:content/jcr:mimeType!=video`」の&#x200B;**DAM アセットの更新ワークフロー –**&#x200B;の&#x200B;*Node Modified*&#x200B;のランチャーです。「Globbing」の値は次のとおりです。
...
1. APIは`jobId`を返します。 ステータスを確認するには、GET リクエストを送信します。
   ...
   - 完了すると、GETは成功で応答し、マップが失敗したかどうかを示します。
...
1. `damAssetLucene` oak インデックスで、`/oak:index/damAssetLucene/indexRules/dam:Asset`にブール型プロパティ `indexNodeName=true`を追加します。
...
3. `reindex=true`を設定して`damAssetLucene`のインデックスを再作成し、再度`false`になるまで待ちます（時間がかかる場合があります）。
...
1. （オプション）大規模なリポジトリの場合はOak queryLimitReadsを増やします
   ...
   **100,000個を超えるDITA ファイル**&#x200B;がある場合、`org.apache.jackrabbit.oak.query.QueryEngineSettingsService`の`queryLimitReads`をアセット数より大きい値に更新し（例：`200000`）、再配置して続行します。
...
1. すべてのファイルに対して後処理を実行するには、次のAPIを実行します。
   ...
   | 期待される応答 | APIはjobIdを返します。 ジョブのステータスを確認するには、ジョブ IDを含むGET リクエストを同じエンドポイントに送信できます。<br> サンプル URL: `http://<server:port>/bin/guides/reports/upgrade` |

   | 終点 | /bin/guides/reports/upgrade |
   |---|---|
   | リクエストタイプ | **GET** |
   | Param | jobId：以前の投稿リクエストから受信したjobIdを渡します。 |
   | 期待される応答 | - ジョブが完了すると、GET リクエストは成功して応答します。<br> - エラーが発生した場合は、エラーログとAPI出力をカスタマーサクセス チームと共有します。  <br> サンプル URL: `http://<server:port>/bin/guides/reports/upgrade?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678` |
...
- Guidesでは`order=50`が使用されるため、`order`の値&#x200B;**を50**&#x200B;より大きくしてください。
