...
7. [フォルダープロファイルでのスマート提案の設定](./conf-profiles.md#configure-ai-assistant-for-smart-help-and-authoring)
...
1. 「**設定**」タブに切り替えます。
1. JSON サービスの詳細（前のセクションでダウンロードした）を、`SERVICE_ACCOUNT_DETAILS`に対応する&#x200B;**値** フィールドに貼り付けます。 次のスクリーンショットに示すように、同じ名前と設定を使用してください。
