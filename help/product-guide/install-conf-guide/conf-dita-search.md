...
次のタブでは、Experience Manager Guidesの設定に基づいて、検索シリアル化ファイルにカスタム DITA要素または属性を追加する手順を示します。Cloud Serviceまたはオンプレミス。
...
1. Adobe Experience Manager Web コンソールの設定ページを開きます。 設定ページにアクセスするためのデフォルトのURLは次のとおりです。
...
次のタブには、Experience Manager Guidesの設定に基づいて`configMgr`のメタデータ抽出オプションを有効にする手順が示されています。Cloud Serviceまたはオンプレミス。

>[!BEGINTABS]

>[!TAB Cloud Service]

設定ファイルを作成するには、[設定の上書き](download-install-config-override.md#)の手順を使用します。 設定ファイルで、メタデータ抽出オプションを設定するために次の\（property\）詳細を指定します。
...
次のタブには、Experience Manager Guidesの設定に基づいて、一時的な翻訳フォルダーを検索から除外する手順が表示されます。Cloud Serviceまたはオンプレミス。
...
>[!NOTE]
>
> この手順を使用して、除外リストに他のフォルダーの場所を追加できます。

1. AEMにログインし、CRXDE Lite モードを開きます。

1. 次の場所にあるdamAssetLucene ノードに移動します。
...
1. lucene ノードに次のプロパティを追加します。
   ...
   | プロパティ名 | タイプ | 値 |
   |-------------|----|-----|
   | excludedPaths | String\[\] | 次の値をこのプロパティに追加します：<br><ul><li>/var/dxml</li><li>/content/dam/projects/translation\_output</li></ul> |
