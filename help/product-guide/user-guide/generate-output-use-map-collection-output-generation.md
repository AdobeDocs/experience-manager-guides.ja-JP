...
1. 「**完了**」をクリックします。
   ...
   ![コレクションダッシュボードをマップ](./images/map-collection-dashboard.png){width="800" align="left"}
...
マップコレクションで、DITA マップのメタデータプロパティを一括で設定できます。 選択 **メタデータを設定**  開く **アセットメタデータ** ページに貼り付けます。 次の日： **アセットメタデータ** ページの場合、コレクションに存在するすべてのマップが左側に表示されます。

![メタデータの設定](images/map-collection-asset-metadata.png){width="800" align="left"}

メタデータのプロパティを設定するには、次の手順を実行します。

1. メタデータを更新するマップを選択できます。 デフォルトでは、存在するすべての DITA マップが選択されます。

1. DITA マップを選択すると、メタデータ、スケジュール（無効）アクティベーション、参照、ドキュメントの状態などのプロパティを表示できます。

1. メタデータのプロパティを更新します。

1. クリック **保存して閉じる** をクリックして、更新内容を保存します。
1. （オプション）タグを更新する際に、「追加」を選択して、 **保存して閉じる** 既存のリストに新しいタグを追加するドロップダウン。
1. クリック **送信** から **保存して閉じる** ドロップダウン。
マップコレクションから一括で選択した DITA マップのメタデータプロパティが更新されます。

>[!NOTE]
> 
>の **ドキュメントの状態** ドロップダウンで、選択したすべての DITA マップで共通に使用できる文書状態のみを選択できます。 詳しくは、 [**ドキュメントの状態**](./web-editor-document-states.md).

メタデータのプロパティがファイルのプロパティと同期している。 更新後は、 **ファイルのプロパティ** パネルを使用して、Web エディターで設定できます。
