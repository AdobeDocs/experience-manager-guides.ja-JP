...
- **メタデータ**：トピックのバージョンメタデータと、その値を制御して、 **バージョン履歴** ダイアログボックス。  メタデータパスで、メタデータを選択するノードの場所を指定します。 また、ラベルとしてメタデータのカスタム名を定義することもできます。 デフォルトのプロパティは、「タイトル」、「ドキュメントの状態」、「タグ」です。

  メタデータは、 `/jcr:content` アセットのノードに追加して、プロパティのパスをメタデータパスとして追加できます。


  メタデータパスが空白の場合は、エラーが表示されます。 ラベルを空白のままにした場合、最後の要素がラベルとして選択されます。
  ...
  *のメタデータの設定&#x200B;**バージョン履歴**ダイアログボックス。*




  また、これらのメタデータタグの表示順を定義することもできます。 これらのタグのデフォルトの順序を変更するには、点線のバーを選択して、目的の場所にタグをドラッグ&amp;ドロップします。
メタデータラベルは、 **バージョン履歴** 」をクリックします。
...
- **次の基準でファイルを表示**:Web エディターでファイルを表示するデフォルトの方法を選択します。 内の様々なパネルから、タイトル別またはファイル名別にファイルのリストを表示できます。 **作成者** 表示。
  >[!NOTE]
  >
  > デフォルトでは、ファイルは Web エディターにタイトル別に表示されます。
...
現在の有効な場所または次の有効な場所に有効な要素を挿入します。 キーボードショートカットを使用することもできます ***Alt***+***入力*** をクリックして、「要素を挿入」ポップアップを開きます。 例えば、段落を編集し、 **要素を挿入** ポップアップが表示され、段落に挿入可能な要素のリストが表示されます。 挿入する要素を選択します。 キーボードを使用して要素のリストをスクロールし、 ***入力*** をクリックして、必要な要素を挿入します。 また、検索ボックスに文字や文字列を入力し、その文字を基にした要素を検索することもできます。


![要素を挿入](images/insert-element.png){width="300" align="left"}

*「t」と入力して、「t」で始まる有効な要素をすべて検索します。*

ブロック要素内で `note`の後に、「要素を挿入」アイコンを使用して、 `note` 要素を選択します。 次のスクリーンショットでは、p 個の\(paragraph\) 要素の内側にメモ要素が挿入されています。
...
新しい要素を入力する別の方法は、コンテキストメニューを使用することです。 ドキュメント内の任意の場所で右クリックして、コンテキストメニューを起動します。 このメニューから、を選択します。 **要素を挿入** 表示する **要素を挿入** ダイアログボックスを開き、挿入する要素を選択します。
...
>[!NOTE]
>
>また、 **リストを分割** オプションを使用して、現在のリストを分割し、同じレベルで新しいリストを開始することができます。
...
また、複数のセル、行全体またはテーブルの列に対して属性を定義することもできます。 例えば、テーブルのセルを整列させるには、必要なセルをドラッグして選択します。 右側のコンテンツプロパティパネルで、 **タイプ** 次に変更： **複数のエントリ**.

1. Adobe Analytics の **属性** セクションで、 **+追加**.
1. を選択します。 `@valign` 属性 **属性** ドロップダウンリスト。
1. 値ドロップダウンリストから、選択したテーブルセルに適用する目的のテキストの整列を選択します。
1. クリック **を追加します。**
...
また、画像の「配置」設定を「インライン」または「分割」として指定することもできます。 「ブレーク」配置オプションを使用する場合は、イメージの位置揃えを行う場所（「左」、「中央」、「右」）を選択できます。
...
The **バージョン履歴** Web エディターの機能を使用すると、DITA ファイルの使用可能なバージョンを確認し、比較し、エディター自体から任意のバージョンに戻すことができます。

バージョン履歴では、現在のバージョンのコンテンツとメタデータ（作業用コピーも可）を同じファイルの以前のバージョンと比較できます。 比較対象のバージョンのラベルおよびコメントも表示できます。
...
1. 有効にする **ラベルとコメントの表示** オプションを使用して、現在のバージョンと比較対象のバージョンに適用されたラベルおよびコメントを表示できます。

1. また、 **バージョン履歴** ダイアログボックス：

   **プレビュー** tab：新しく追加されたコンテンツは緑のフォントで、削除されたコンテンツは赤のフォントです。

   **メタデータ** タブ：新しく追加されたメタデータは緑色のフォントで、削除されたメタデータは赤色のフォントで表示されます。
   ![バージョンのメタデータの違い ](images/metadata-version-diff.png){width="550" align="left"}
   *バージョン履歴で異なるバージョンのメタデータを比較する。*

   >[!NOTE]
   >
   > システム管理者は、エディター設定の「メタデータ」タブから、表示するメタデータを変更できます。

   また、現在のバージョンと比較対象のバージョンのユーザーと時間の詳細も表示できます。



1. ドロップダウンリストからバージョンを選択すると、 **「選択したバージョンに戻す」オプション** が使用可能になりました。 プレビューウィンドウに、現在のバージョンとトピックの選択したバージョンの違いが表示されます。
...
以前のバージョンに戻すと、現在作業しているバージョンが最新バージョンでないことを示す視覚的なキューが表示されます。
...
デフォルトでは、ファイルはタイトル別に表示できます。 ファイルの上にマウスポインターを置くと、ファイルタイトルとファイルパスがツールチップとして表示されます。
>[!NOTE]
>
> 管理者は、Web エディタでファイル名別にファイルのリストを表示することもできます。 を選択します。 **ファイル名** オプション **次の基準でファイルを表示** のセクション **ユーザーの環境設定** ![](images/user_preference_editor_icon.svg).
...
リポジトリー表示アイコンをクリックすると、DAM で使用可能なファイルとフォルダーのリストが表示されます。 デフォルトでは、ファイルはタイトル別に表示できます。 ファイルの上にマウスポインターを置くと、ファイルのタイトルとファイル名がツールチップとして表示されます。
...
マップを選択し、[Enter] を押すかダブルクリックして、 **マップビュー**. 詳しくは、 **マップビュー** 機能の説明 [左パネル](web-editor-features.md#id2051EA0M0HS) 」セクションに入力します。 トピックを選択し、Enter キーを押すかダブルクリックして、 [コンテンツ編集領域](#id2051EB000UI). Web エディターから直接ファイルを移動して開くことで、時間を節約し、生産性を高めます。
...
マップビューアイコンをクリックすると、マップファイル内のトピックのリストが表示されます。 マップファイルを開いていない場合は、[ マップビュー ] は空白で表示されます。 任意のマップファイルをダブルクリックすると、このビューでマップファイルが開きます。 マップ内の任意のファイルをダブルクリックして、Web エディタで開くことができます。

デフォルトでは、ファイルはタイトル別に表示できます。 ファイルの上にマウスポインターを置くと、ファイルタイトルとファイルパスがツールチップとして表示されます。
>[!NOTE]
>
>管理者は、現在マップビューで開いている親マップのファイル名を表示することもできます。 を選択します。 **ファイル名** オプション **次の基準でファイルを表示** のセクション **ユーザーの環境設定** ![](images/user_preference_editor_icon.svg).


マップビューでマップを開くと、現在のマップのタイトルがメインツールバーの中央に表示されます。 タイトルが長すぎる場合は、省略記号が表示され、タイトルの上にマウスポインターを置くと、完全なタイトルがツールヒントに表示されます。

トピックまたはマップ参照のキー属性を定義すると、タイトル、対応するアイコンおよびキーを左側のパネルに表示できます。 キーは次のように表示されます。 `keys=<key-name>`.

![マップビューのキー](images/view-key-title-map-view.png){width="300" align="left"}

マップファイルに対する編集権限がある場合は、ファイルも編集できます。 DITA マップを使用したトピックの開封と編集について詳しくは、 [DITA マップを使用したトピックの編集](map-editor-advanced-map-editor.md#id17ACJ0F0FHS).
...
- **マップをダウンロード**：このオプションを選択して、 **マップをダウンロード** ダイアログボックス。
Adobe Analytics の **マップをダウンロード** ダイアログボックスでは、次のオプションを選択できます。
  ...
  次に、 **ダウンロード** 」ボタンをクリックした場合、マップ書き出しパッケージ要求がキューに入ります。 The **成功** パッケージが正常に作成されると、ダイアログボックスが表示されます。  次の項目をクリックします。 **ダウンロード** ボタンを **成功** ダイアログボックス。

  マップをダウンロードする準備ができたら、マップのダウンロード準備完了の通知を受け取ります。 ダウンロードが失敗した場合は、マップのダウンロードに失敗したという通知を受け取ります。
...
- The **オプション** メニュー *アウトライン表示* を使用すると、現在の要素の前後に、切り取り、コピー、削除、ID の生成、要素の挿入、要素の名前変更または置換、要素の折り返し、選択した要素からのスニペットの作成などの一般的な操作を実行できます。
...
また、 **属性** ドロップダウン **コンテンツのプロパティ** パネル **作成者** 表示。
![](images/subject-scheme-attribute-dropdown.png){width="200" align="left"}
値を変更するには、次の手順を実行します。

1. 次の中から属性を選択： **属性** ドロップダウン。
1. 選択 **編集** ![edit-icon](images/edit_pencil_icon.svg).
1. 必要な値を **値** ドロップダウン。
1. 「**更新**」をクリックします。
...
デフォルトでは、ファイルはタイトル別に表示できます。 テンプレートの上にマウスポインターを置くと、ファイルのタイトルとファイル名がツールチップとして表示されます。

>[!NOTE]
>
> 管理者は、Web エディタでファイルのリストを表示することもできます。 を選択します。 **ファイル名** オプション **次の基準でファイルを表示** のセクション **ユーザーの環境設定** ![](images/user_preference_editor_icon.svg).
...
レビューパネルにレビュータスクが表示されます。 デフォルトでは、ファイルはタイトル別に表示できます。 ファイルの上にマウスポインターを置くと、ファイルタイトルとファイルパスがツールチップとして表示されます。
>[!NOTE]
>
> 管理者は、Web エディタでファイル名別にファイルのリストを表示することもできます。 を選択します。 **ファイル名** オプション **次の基準でファイルを表示** のセクション **ユーザーの環境設定** ![](images/user_preference_editor_icon.svg).
...
>[!NOTE]
>
> 管理者は、Web エディタでファイル名のリストを表示することもできます。 を選択します。 **ファイル名** オプション **次の基準でファイルを表示** のセクション **ユーザーの環境設定** ![](images/user_preference_editor_icon.svg).
...
コンテンツ編集領域は、トピックまたはマップのコンテンツが表示される場所です。 この領域では、すべてのコンテンツを編集します。 編集中のコンテンツが WYSIWYG 形式で表示されます。 複数のトピックを同時に開くことができ、それぞれのタブに表示されます。

デフォルトでは、ファイルタイトルはタブに表示されます。 ファイルの上にマウスポインターを置くと、ファイルタイトルとファイルパスがツールチップとして表示されます。
>[!NOTE]
>
> 管理者は、タブでファイル名別にファイルのリストを表示することもできます。 を選択します。 **ファイル名** オプション **次の基準でファイルを表示** のセクション **ユーザーの環境設定** ![](images/user_preference_editor_icon.svg).

「ファイル」タブの下に、現在のカーソル位置にある要素のパンくずリストが表示されます。 コンテンツ編集領域の右上隅に、現在のトピックのバージョン番号が表示されます。
...
コンテンツプロパティ機能にアクセスするには、右側のパネルでコンテンツプロパティアイコンをクリックします。 コンテンツのプロパティパネルには、ドキュメント内で現在選択されている要素のタイプとその属性に関する情報が含まれます。
属性の追加、編集、削除を簡単に行うことができます。

1. クリック **+追加**.

   ![コンテンツプロパティの属性](images/properties-tab-attributes_cs.png){width="300" align="left"}

1. Adobe Analytics の **属性** ドロップダウンパネルで、ドロップダウンリストから属性を選択し、属性の値を指定します。  次に、「 **追加**.

   ![複数の属性を持つ属性パネル ](images/attributes-multiple-properties.png){width="300" align="left"}

1. 属性を編集するには、その属性にカーソルを合わせて「 」を選択します。 **編集** ![edit-icon](images/edit_pencil_icon.svg).
   ![属性を編集](images/edit-attributes-content-properties.png){width="300" align="left"}

1. 属性を削除するには、その属性にカーソルを合わせ、「 」を選択します。 **削除** ![delete-icon](images/Delete_icon.svg).
...
[ ファイルのプロパティ ] をクリックして、選択したファイルのプロパティを表示します。 ![](images/topic-properties-icon.svg) アイコンが右側のパネルに表示されます。 ファイルのプロパティ機能は、レイアウト、作成者、ソース、プレビューの 4 つのモードまたはビューで使用できます。

[ ファイルのプロパティ ] には、次の 2 つのセクションがあります。
...
- **タグ**：トピックのメタデータタグです。 これらは、プロパティページの「タグ」フィールドから設定されます。 ドロップダウンから入力または選択できます。  タグはドロップダウンの下に表示されます。 タグを削除するには、タグの横にあるバツ印アイコンを選択します。
- **その他のプロパティを編集**：ファイルのプロパティページで、さらにプロパティを編集できます。
