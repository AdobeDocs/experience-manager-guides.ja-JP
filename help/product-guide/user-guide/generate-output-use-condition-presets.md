...
- [マップ コンソールから](#condition-presets-from-the-map-console)
- [マップ ダッシュボードから](#condition-presets-from-the-map-dashboard)
...
1. 次のいずれかの操作を行って、属性を追加します。
   ...
   - **すべてを選択**：すべての属性とその値を条件プリセットに追加します。 デフォルトでは、属性のアクションは `Include` です。

1. 「**追加**」を選択します。この手順を繰り返して、さらに属性を追加できます。 追加する属性は、中央のパネルから右側のパネルに移動します。
1. 上部のアクションバーから「**削除**」を選択して、選択した属性を右側のパネルに表示して削除します。
...
条件プリセットを作成すると、その条件プリセットが **出力プリセット** の **条件プリセット** ドロップダウンの下に表示されます。 詳しくは、[PDF出力の公開 ](../web-editor/native-pdf-web-editor.md) 方法を参照してください。
...
1. **条件プリセット** パネルの条件プリセットの上にマウスポインターを置きます。
1. オプションメニューから「**複製**」を選択して **条件プリセットを複製** ダイアログボックスを開きます。
...
1. **条件プリセット** パネルの条件プリセットの上にマウスポインターを置きます。
1. オプションメニューから **削除** を選択して、**条件プリセットを削除** ダイアログボックスを開きます。
1. 「**削除**」を選択します。
...
1. 次のいずれかの操作を行って、属性を追加します。
   - 条件プリセットに対して 1 つの属性に **追加** を選択します。 この手順を繰り返して、さらに属性を追加できます。
   - **すべて追加** を選択して、すべての属性を条件プリセットに追加します。
1. \（オプション\）必要に応じて、手順 4 で属性に適用したデフォルトのアクションを上書きできます。 次のいずれかの操作を行います。
   - 複数の属性を選択し、**選択した条件のアクションを設定** からアクションを選択して、「**適用**」を選択します。
