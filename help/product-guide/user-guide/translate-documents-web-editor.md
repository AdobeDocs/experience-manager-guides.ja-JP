...
1. **適用** を選択します。 トピックと関連アセットの詳細を含むリストが表示されます。
   ...
   >[!NOTE]
   >
   > DITAVAL および Markdown ファイル参照を使用してマップを変換する場合、および作業用コピーに基づいて変換する場合、イメージやその他のリンクされたアセットなどの参照は、ソース言語フォルダに存在する限り含まれます。 これらの参照は、翻訳ダッシュボードの参照リストに表示され、翻訳用に明示的に選択できます。 翻訳時に、参照されるアセットがターゲット言語フォルダーにコピーされ、標準の画像翻訳動作で一貫して処理されます。
...
1. 翻訳のために送信するトピックを選択します。 次の列に対して、トピックのフィルタリングオプションを使用することもできます。
   ...
   - **バージョン**：ソースファイルのバージョン番号
     ...
     ファイルにバージョン管理されていない変更が保存されている場合（つまり、マップ内の新しいバージョンとして保存されていない場合）、バージョン管理されていない変更があることを示す情報アイコンがファイルの横に表示されます。

     ![](images/unversioned-changes-translation.png){width="650" align="left"}

     >
     >
     > バージョン管理されていない変更を含むファイルのみを表示するには、フィルターパネルで **バージョン管理されていない変更を含むアセットのみを表示** 設定を有効にします。 また、バージョン管理されていないインジケータは、最新バージョンに基づいてファイルを翻訳する場合にのみ表示されます。
...
ターゲットコピーにソースバージョンラベルを追加するには、システム管理者は、**Workspace設定の** 翻訳 **タブにある** ソースバージョンラベルをターゲットバージョンに伝播 **オプションを有効にする必要があります** （**オンプレミス** の場合は **設定** と表示されます）。
...
1. 「**同期を強制**」を選択します。 選択した非同期アセットのステータスを同期中に設定します。
...
管理者は、&lbrace;4 **Workspace設定** の「**翻訳**」タブ（「オンプレミス **の場合は「** 設定 **と表示されます）で「** 完了後の翻訳プロジェクトのクリーンアップ **」オプションを設定して、翻訳プロジェクトを自動的に無効にしたり削除したりできます。**
