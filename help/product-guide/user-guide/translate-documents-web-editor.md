...
1. 翻訳のターゲットとして、任意の言語グループを選択できます。 **すべてを選択**&#x200B;すると、選択したファイルは、既存の言語グループ内のすべての使用可能な言語に翻訳されます。
   ...
   >[!NOTE]
   >
   > 個別のソース言語または宛先言語コピー（ファイル）が元の言語フォルダーから別のフォルダーに移動された場合、翻訳パネルには次のように変更が反映されます。
   > - 宛先言語コピー：宛先（ターゲット）言語コピーが元の宛先言語フォルダーから別のフォルダーに移動された場合、対応するアセットは翻訳パネルで無効と表示されます。
   > - Source言語コピー：元のソース言語フォルダーから別の言語フォルダーに移動すると、対応するアセットが翻訳パネルに有効と表示され、翻訳が期待どおりに続行されます。
...
1. ドロップダウンから、**新しい翻訳プロジェクトを作成**&#x200B;を選択します。
   ...
   - 既に翻訳プロジェクトがある場合は、そのプロジェクトにトピックを追加できます。 プロジェクト リストから「**既存の翻訳プロジェクトに追加**」オプションを選択し、既存の翻訳プロジェクト リストからプロジェクトを選択します。 これらのプロジェクトは、最新、昇順、降順で並べ替えることができます。
     ...
     >[!NOTE]
     >
     > **既存の翻訳プロジェクトに追加**&#x200B;を使用して翻訳用の追加アセットを送信する場合、同じプロジェクトの以前のリクエスト（プロジェクト作成または既存のプロジェクトに追加）がまだ進行中の場合、操作は失敗します。 次のメッセージが表示されます：*このプロジェクトに対する以前のリクエストはまだ処理中です。 数分待ってから、もう一度やり直してください*。
...
1. **パス** フィールドで、新しい翻訳プロジェクトを作成する場所を選択します。
   ...
   - 既存の翻訳プロジェクトに追加する場合は、そのプロジェクトが既に存在するパスを選択します。 次に、**既存の翻訳プロジェクト** リストから、現在の翻訳タスクを追加するプロジェクトを選択します。 リストを絞り込むには、**フィルター**&#x200B;を使用することもできます。
     ...
     ![既存のプロジェクト パスを入力](images/existing-project-path.png){width="650"}
...
ターゲットコピーにソースバージョンラベルを追加するには、システム管理者が、**Workspace settings**&#x200B;の「**翻訳**」タブで「**ソースバージョンラベルをターゲットバージョン**」オプションを有効にする必要があります（バージョン 5.2 **より前の** オンプレミス設定の場合は&#x200B;**Settings**&#x200B;と表示されます）。
