...
>[!NOTE]
>
> - この記事は、Experience Manager Guides 2026.04.0 リリースで利用可能なパフォーマンスと安定性の向上を提供する、現在&#x200B;*Beta*&#x200B;機能として利用可能な新しいベースラインに適用されます。
> - Cloud Serviceの新しいベースライン機能を有効にするには、カスタマーサクセス チームにお問い合わせください。
> - オンプレミスの新しいベースライン機能を有効にするには、[新しいベースラインの設定](../install-conf-guide/conf-new-baseline-on-prem.md)を表示します。
...
| 領域 | 変更（説明） |
|------|-------------|
| **参照解決** | ダイレクトマップ参照は&#x200B;**DIRECT**&#x200B;に分類されます。 無効な参照はスキップされ、`reltable`からの参照は引き続き除外されます。 これは、編集されたベースラインと新しいベースラインでサポートされますが、移行されたベースラインではサポートされません。 |
| **自動的に選択** | バージョン選択は、正確なバージョン解決を確実にするために、直接参照を解決する直前に評価されます。 これは、編集されたベースラインと新しいベースラインでサポートされますが、移行されたベースラインではサポートされません。 |
...
| **UI** | 動的なベースラインを表示し、参照バージョンの編集を効率化できます。 |
...
1. ダイアログに次の詳細を入力します。
   ...
   1. **フォルダーとファイルを選択**：移動して、処理する1つまたは複数のフォルダーとファイルを選択します。 ベースラインの移行用に選択できるのはフォルダーのみです。

      >[!NOTE]
      >
      > すべてのガイドコンテンツとマップファイルを含むフォルダーを選択します。 マップファイルが個別に保存されている場合は、マップファイルが格納されているディレクトリを選択します。
