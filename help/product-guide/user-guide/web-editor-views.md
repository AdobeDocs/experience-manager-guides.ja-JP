...
* [ベースラインを使用したコンテンツのプレビュー](#preview-content-using-baseline)
* [条件付きフィルターに基づくコンテンツの表示](#id2114BI00VXA)
* [変更履歴のマークアップの表示](#id2114BJ00CE8)
* [トピックをPDFとして書き出す](#id2114BL00B5U)


### ベースラインを使用したコンテンツのプレビュー（DITA マップにのみ適用）

>[!NOTE]
>
>お客様の環境でこの機能を有効にするには、カスタマーサクセス チームにお問い合わせください。

デフォルトのプレビューモードでは、トピックまたはマップの現在の作業コピーが表示されます。 マップに1つ以上の静的ベースラインがある場合、「**ベースラインを使用してプレビュー**」トグルを使用して、特定のベースラインバージョンに存在するマップを代わりにプレビューできます。 この機能は静的ベースラインのみをサポートしています。動的ベースラインはプレビューできません。

![](images/preview-baseline.png){width="650"}

ドロップダウンリストからベースラインを選択すると、現在の作業コピーではなく、選択したベースラインに基づいて、関連するすべてのトピック、アセット、画像、参照がプレビューによって解決されます。 これにより、ベースライン作成時のマップコンテンツを正確に表現できます。

例えば、ベースラインを作成した後に画像が置き換えられた場合、そのベースラインをプレビューすると、現在の作業コピーの画像ではなく、ベースラインに関連付けられている画像のバージョンが表示されます。

プレビューモードでは、トピックの初期セットがレンダリングされる間、読み込みプレビューインジケーターが表示されます。 トピックを編集したり、バージョンを切り替えたりしても、自動的には更新されません。 最新のコンテンツを表示するには、プレビューを手動で更新するか、トピックまたはマップを閉じて再度開きます。 また、ベースラインのマップをプレビューする際に、トラッキングオプション（マークアップなし、オリジナル、マークアップを表示）は使用できません。
...
次の画像は、2つの条件（`Audience`と`Platform`）を使用するトピックを示しています。 コンディショナライズされたコンテンツが黄色の背景で強調表示されます。
...
* **元の**：このビューでは、すべての挿入が拒否され、すべての削除が元に戻され、プレビューが表示されます。 変更をトラック モードを有効にする前に、ドキュメントの元のフォームを取得します。
