...
ユーザーにタグを付けるには、コメントまたは返信に`@`と入力し、レビュー担当者のリストからユーザーを選択します。 ユーザのメールアドレスと共に表示名を表示し、識別を容易にする。 メールアドレスが利用できない場合は、代わりにユーザーIDが表示されます。
...
>[!TAB 条件パネル （DITAVAL ベースの条件なし） ]

![](./images/review-conditions-panel.png){width="350"}

>[!TAB 条件パネル （DITAVAL ベースの条件付き） ]
