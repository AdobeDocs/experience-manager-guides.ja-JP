...
| JSON オプション | 説明 |
| --- | --- |
| 出力パス | JSON 出力が保存されるAEM リポジトリ内のパス。 出力パスは、管理者が設定する変数 `${base_output_path}` を使用して設定されます。 出力パスを設定するには、使用しているサービスに応じて、[ クラウドサービスのベース出力場所を設定する ](../native-pdf/configure-base-location-cs.md) または [ オンプレミスのサービスのベース出力場所を設定する ](../native-pdf/configure-base-output-location.md) を表示します。 |
