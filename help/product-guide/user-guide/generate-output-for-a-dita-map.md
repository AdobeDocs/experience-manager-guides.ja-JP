...
>[!NOTE]
>
> マップでDITAVAL ファイルを使用する場合、DITAVAL ファイルで参照されているフラグ付きの画像は、出力で公開されたマップに関連する場所にコピーされます。  また、同じマップ内で複数のDITAVAL ファイルをフィルタリングに使用する場合は、公開中にファイル名が重複するのを避けるために、一意の`.ditaval` ファイル名を使用するようにしてください。
