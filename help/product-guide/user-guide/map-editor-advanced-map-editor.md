...
エディターツールバーに表示されるオプションは、マップエディタービューに基づいています。 マップエディターでは、次の5つのビューを使用できます。

- [レイアウト](#layout-view)
- [作成者](#author-view)
- [ソース](#source-view)
- [プレビュー](#preview)
- [並べて表示](#side-by-side-view-available-in-new-editor-only)
...
>[!NOTE]
>
> 新しいエディターでは、**レイアウト**&#x200B;機能は現在サポートされていません。
...
**作成者** ビューでは、エディターでDITA マップを編集できます。 これは、マップエディターのWYSIWYG ビューを示しています。
...
**メニュー** ドロップダウンで使用できるオプションは、[ エディターツールバー](./web-editor-toolbar.md#menu-dropdown)で使用できるオプションと同じです。

さらに、次のアイコンを表示し、作成者ビューから関連するタスクを実行できます。

**挿入前/挿入後** - ![](images/insert_element_before_icon.svg) / ![](images/insert_element_after_icon.svg)

**挿入前**&#x200B;または&#x200B;**挿入後** ダイアログボックスを表示します。 マップに挿入するエレメントを選択します。 操作に応じて、新しいエレメントはマップ内の現在のエレメントの前または後に挿入されます。
...
>[!NOTE]
>
> 新しいエディターでは、**タグ**&#x200B;を追加または削除するオプションをエディター設定で利用できます。 詳しくは、[ エディター設定](./config-editor-settings.md)を参照してください。
...
>[!NOTE]
>
> 新しいエディターでは、**変更履歴**&#x200B;機能は現在サポートされていません。
...
>[!NOTE]
>
> このセクションは、古いエディターにのみ適用されます。 新しいエディターでは、マップとそのトピックが自動的に更新されます。
...
## 並べて表示（新しいエディターでのみ使用できます）

新しいエディターの並べて表示すると、作成者ビューとSource ビューを並べて表示できます。 これにより、マップの構築方法を容易に把握でき、変更が意図した構造に沿ったものになります。

例えば、出力にトピックが正しく表示されない場合は、作成者ビューを使用してマップ内の配置を確認すると同時に、Source ビューを確認してトピック参照または属性を確認できます。
...
1. エクスプローラーに移動し、編集するマップファイルを開きます。

   マップファイルがマップビューで開きます。

   >[!NOTE]
   >
   > Assets UIを使用して、マップファイルを開くこともできます。 編集するトピックを含むDITA マップファイルに移動し、メインツールバーの「**トピックを編集**」を選択してエディターを起動します。

1. 任意のトピックを選択してエディターで開き、編集します。
...
コンテンツプロパティパネルには、マップ内で現在選択されているトピック（タイプ、リンクパス、リンク UUID、属性など）に関する情報が含まれます。

詳細については、エディターで[ コンテンツのプロパティ ](web-editor-features.md#right-panel)を参照してください。
...
以下のタブを参照して、新しいエディターと古いエディターのオーサービューでの属性の表示方法を比較します。

>[!BEGINTABS]

>[!TAB 新しいエディター]

![作成者ビュー](images/author-inline-attributes-editor-2-0.png){width="650"}

>[!TAB 古いエディター]

![作成者ビュー](images/author-inline-attributes.png){width="650"}
