...
マップ エディタのツールバーは、トピック エディタに似ています。 左側のパネルの切り替え、マップの保存、新しいバージョンのマップの作成、最後の操作の取り消しとやり直し、選択した要素の削除などの基本的な操作は、両方のエディターで共通です。 これらの操作の仕組みについて詳しくは、[&#x200B; エディターのツールバー &#x200B;](web-editor-toolbar.md#) の節を参照してください。
...
[ ファイルを選択 ] ダイアログ ボックスが表示されます。 挿入するトピック/マップ ファイルに移動し、[**選択**] を選択してマップに追加します。
...
マップをプレビューモードで開き、エディタの別のタブからマップに切り替えた場合、プレビューは自動的には更新されません。 マップのプレビューを再ロードするには、手動で更新する必要があります。

マップのプレビューモードで使用できる更新機能は、次の 2 つの方法で使用できます。

- **マップレベルの更新**:「**更新**」ボタンを選択すると、マップ全体（すべてのトピックを含む）のプレビューが更新され、最初のスクロール位置に移動して表示が明確になります。
  ![](images/refresh-map.png){width="650" align="left"}

- **トピックレベルの更新**：コンテキストメニューから **トピックを更新** を選択すると、スクロール位置を変更せずに、その特定のトピックが更新されます。
