---
title: エディターのツールバー
description: エディターのツールバーについて説明します。 エディターのインターフェイスと機能については、Adobe Experience Manager ガイドで説明します。
...
>[!NOTE]
>
> 上記の機能は、トピックファイルにのみ適用されます。 マップ ファイルを操作する場合、現在のマップ 編集者 表示に基づいて、ツールバーにさまざまなオプションが表示されます。 [ マップエディター機能 ](./map-editor-advanced-map-editor.md) ドキュメントのマップエディターツールバーオプションについて説明します。
...
ラベルは、特定のトピックが DDLC \(ドキュメント開発ライフサイクル\) にあるステージを識別するのに役立ちます。 例えば、トピックに取り組んでいる場合、ラベルを「承認済み」に設定できます。 トピックが公開され、顧客が使用できるようになったら、そのトピックに「リリース済み」ラベルを割り当てることができます。

Experience Manager Guidesでは、自由形式のテキスト形式でラベルを指定したり、事前定義済みの一連のラベルを使用したりできます。 カスタムラベルを使用すると、システム内の任意の作成者が選択したラベルを指定できます。 これにより柔軟性が生まれますが、システム内でラベルの一貫性が失われます。 この問題を解決するために、管理者は、事前定義済みのラベルのセットを設定できます。 定義済みラベルの構成の詳細については、「Adobe Experience Manager ガイドをCloud Serviceとしてインストールおよび構成表示 *XML Webエディターの構成とカスタマイズ* を参照してください。

これらのラベルは、作成者がラベルを指定する必要がある場合に、ドロップダウン リストの形式で表示されます。 これにより、事前定義された一貫性のあるラベルのみがシステムで使用されるようになります。

トピックにラベルを適用するには、 [バージョン履歴](web-editor-use-label.md) Assets UIのパネル、 [ベースライン](/help/product-guide/user-guide/generate-output-use-baseline-for-publishing.md) UI、エディターなど、さまざまな方法があります。 エディターのバージョンラベル機能を使用すると、作成者はトピックにラベルをすばやく簡単に割り当てることができます。
...
1. ラベルを適用するバージョンを選択します。
   ...
   バージョンリストとは異なるバージョンのトピックを選択すると、プレビューパネルに、トピックの現在のバージョンと選択したバージョンの間の変更が表示されます
...
1. オプションから、次のいずれかを選択します。

   - **選択したバージョンからの変更の追跡**: このオプションでは、すべての内容更新が変更履歴の形式で表示されます。 その後、ドキュメント内の変更を 1 つずつ承認または却下することも、まとめて承認または却下することもできます。
...
「変更を追跡」モードを有効にすると、ドキュメントに対して行われたすべての更新を追跡できます。 トラックの変更を有効にすると、すべての挿入と削除がドキュメントに取り込まれます。 削除されたすべてのコンテンツは取り消し線でハイライト表示され、すべての挿入は緑色のテキストでハイライト表示されます。 さらに、トピックページの端に変更バーも表示されます。 ここでも、削除された内容には赤いバーが表示され、追加された内容には緑のバーが表示されます。 同じ行に追加と削除がある場合は、緑と赤の両方のバーが表示されます。
...
Experience Manager Guidesには、ドキュメント内で行われた更新に関する情報を含む、変更履歴の保存機能が用意されています。 変更履歴機能は、どのような更新が行われたか、誰がいつ行ったかに関する情報を提供します。 変更履歴機能を使用すると、ドキュメントで提案された更新を簡単に受け入れたり、却下するしたりすることもできます。

この機能にアクセスするには、右側のパネルの **変更の追跡** アイコンを選択します。

![](images/changes-panel_cs.png){width="300" align="left"}

変更履歴を選択すると、ドキュメント内の変更された内容が選択されます。 「変更を許可」アイコンを選択して変更を許可するか、「変更を拒否」を選択して変更を拒否できます。
...
エディタの **タグ** 機能は、DITA エレメントの表示を制御するトグルボタンです。 有効にすると、コンテンツ内に構造タグが表示され、基になる DITA エレメントをより効果的に表示および管理できるようになります。 無効にすると、編集者これらのタグが非表示になり、よりクリーンで焦点を絞ったオーサリング環境が提供されます。

次のスクリーンショットは、タグ 表示が有効になっているドキュメントを示しています。

![](images/tags-view.png){width="650" align="left"}

タグを使用したドキュメントでは、次の操作を実行できます。
...
段落要素を現在または次の有効な位置に挿入します。

**箇条書きのリスト** - ![](images/BulletList_icon.svg)

現在または次の有効な位置に箇条書きリストを作成します。 箇条書きリストで、このアイコンを選択すると、項目は通常の段落に変換されます。
...
テーブルを挿入すると、コンテキストメニューを使用してテーブルを変更できます。
...
<details>
    <summary> 複数のセル、テーブルの行全体または列に対する属性の定義 </summary>

複数のセル、テーブルの行全体または列に属性を定義することもできます。 例えば、テーブルのセルを揃えるには、必要なセルをドラッグして選択します。 コンテンツのプロパティパネル（右側）で、プロパティ **タイプ** が **エントリ** に変わります。
...
現在または次の有効な位置に画像を挿入します。 画像 アイコンを選択して [挿入 画像] ダイアログ ボックスを開き、挿入するイメージを検索して選択します。
...
さまざまな種類のマルチメディア ファイルを挿入します。 [マルチメディア]ドロップダウンアイコンを選択し、挿入するファイルの種類を選択します。 サポートされるマルチメディア形式は次のとおりです。
...
- **コピー UUID**:挿入された参照の UUID をコピーします。
- **コピー パス**: 挿入された参照の完全パスをコピーします。
...
キーワードはアルファベット順に表示されます。「検索」（Search） ボックスに検索文字列を入力してキーワードを検索することもできます。 検索結果には、ID または値に文字列を含むキーワードが返されます。 DITA マップで定義されているキーワードがこのダイアログにリストされます。 挿入するキーワードを選択し、「 **挿入**」を選択します。

キーワードを右クリックして属性オプションを選択して、挿入されたキーワードの属性を変更することもできます。 キーワードの **属性 ダイアログボックスが開きます。
...
データソースに接続し、データを使用して内容を作成します。 [データソースからデータを使用](./web-editor-content-snippet.md)する方法を参照してください。

## 新しいバージョンとしてバージョン情報と保存
