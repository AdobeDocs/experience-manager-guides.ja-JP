---
title: Experience Manager Guidesのエディター設定
...
エディターの **タブバー ](./web-editor-tab-bar.md) に表示される [ 設定** オプションは、管理者およびフォルダープロファイル管理者のみが使用でき、次の設定を指定できます。
...
一般設定を使用すると、エディターで使用する辞書を設定できます。 このタブには、**スペルチェック**、**条件**、**オーサリング**、**引用文献** の 4 つのセクションがあります。
...
- **条件**
   ...
   - **条件属性で検証**: 属性に定義されている値検証を許可する場合に選択します。 これにより、不正な値が追加されなくなります。

   - **件名スキームパネルにタイトルを持つ表示キー**: これを選択すると、サブジェクトスキームのタイトルとともにキーが表示されます。 このオプションを選択しない場合は、タイトルのみが表示されます。 例えば、ここでは「os」、「audience」、「other」のキーもタイトルと共に表示されます。
...
この設定は、エディターコンソールとマップコンソールの左右のパネルに表示されるパネルを制御します。 ボタンを切り替えて、目的のパネルを表示または非表示にするできます。

![](images/editor-setting-panel.png){width="650" align="left"}

また、パネルに存在するフィーチャが表示される順序を定義することもできます。 パネル内の使用可能なフィーチャの既定の順序を変更するには、点線のバーを選択して、フィーチャ タブを目的の位置にドラッグ アンド ドロップします。 機能は、要件に応じて、**その他** セクションからパネルのメインセクションに、またはその逆に移動することもできます。 並べ替え後、各左右のパネルで同じ順序で機能が表示されます。
...
**条件** の事前設定済みの背景色のリストを表示します。 ユーザーは、トピックに条件を適用するときに背景色を選択できます。 管理者は、カスタム背景色を作成してリストに追加することもできます。 新しい色を追加するには、[ **カラー 名前** ] フィールドに目的の名前を入力し、カスタム色を選択して、[ **+** ] アイコンを選択します。 カスタムカラーは、カラーリストの最後に表示されます。

## Publishプロファイル

これには、 **ナレッジベース** 出力公開するために使用できるプロファイルが含まれています。 ターゲットのナレッジベース用に新しいプロファイルを作成できます。 例えば、Salesforceや ServiceNow などです。
...
- プロファイルを作成するには、[ **サーバータイプ** ] ドロップダウンから ServiceNow ナレッジ ベースを選択します。 プロファイル **名前** を入力します。 **ServiceNow URL** で、出力の発行に使用するコンシューマー サイトを入力し、ServiceNow コンシューマー サイトによって提供される **ユーザー名** と **パスワード** を追加します。次に、新しく作成されたプロファイルを **検証** し、 **保存** します。
...
- **すべてのユーザーに検証パネルへの Schematron ファイルの追加を許可する**: エディターの [検証] パネルで任意の Schematron ファイルを追加できるようにするには、これを選択します。 これにより、ユーザーはSchematronファイルを追加し、Schematronファイルに対してトピックを検証できます。 これが選択されていない場合、スキーマトロンファイルを追加するオプション **スキーマトロンファイルを追加** ボタンは、エディターの **検証パネル** のユーザーには使用できません。


## 属性を表示

属性リストと同様に、要素の属性リストに表示する属性のリストを制御できます。 デフォルトでは、要素の属性リストに表示されるように、オーディエンス、プラットフォーム、製品、prop の 4 つの **表示属性** が設定されています。 上部の **追加** アイコンを使用して表示属性を追加することもできます。 「**削除** アイコンを使用して表示属性を削除することもできます。

要素に定義されている属性は、「レイアウト」および「アウトライン」表示に表示されます。
...
- **言語グループ**：管理者は、言語グループを作成し、それらをセットとして使用してコンテンツを翻訳できます。
  ...
  新しい言語グループを作成するには、次の手順を実行します。
   ...
   1. 完了アイコンを選択して、選択した言語をグループに追加します。 言語が表示されます。 3 つ以上の言語を追加すると、[ **さらに表示** ] オプションが表示されます。 **表示さらに**&#x200B;を選択して、グループに存在するすべての言語を表示できます。

      >[!TIP]
      >
      > **表示多い****少ない表示に切り替えて**&#x200B;いくつかの言語のみ表示します。

   1. グループ内の言語の上にマウスポインターを置くと、言語グループを編集 ![編集アイコン](images/edit_pencil_icon.svg) したり、言語グループを削除 ![削除](images/Delete_icon.svg) したりできます。
   1. **設定** を保存します。
