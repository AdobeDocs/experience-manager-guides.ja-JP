...
この機能を使用すると、現在開いているトピックに関連付けられているレビュータスク（開いているか閉じている）をコメントパネルから選択できます。 有効にすると、レビューUIのコメントパネルには、トピックが含まれているすべてのレビュータスクと、各タスクの状態（アクティブまたはクローズ）および属するプロジェクトが一覧表示されるドロップダウンが含まれます。 現在作業中のタスクは、デフォルトで&#x200B;**現在** タグでマークされています。

![](./images/review-task-selection-dropdpwn.png){width="350"}

レビュータスク ドロップダウンを使用すると、レビュープロジェクトを切り替えることなく、これらのタスクのいずれかを選択してコメントを確認できます。 ドロップダウンのタスクの横にある&#x200B;**（i）** アイコンを選択して、タスクの詳細を表示することもできます。

>[!NOTE]
>
> 表示する予定のレビュータスクがドロップダウンに表示されない場合は、この機能がインスタンスで有効になる前にタスクが作成された可能性があります。 問題を解決するには、管理者にお問い合わせください。[ インデックス作成を実行して、トピックのすべてのレビュータスクをコメントパネル ](../install-conf-guide/index-review-tasks.md)に含めます。

この機能を使用する場合は、次の点に注意してください。

- エディターで別のトピックに切り替えると、パネルがコメントの現在のタスクに戻ります
- 現在のタスク以外のタスクを選択すると、そのタスクのトピックのバージョンとコメントが読み取り専用モードで開きます。 コメントを表示できますが、コメントを編集、承認、または却下することはできず、**インポート** オプションは無効になっています。 作業コピーに読み込むことができるのは、現在のタスクのコメントのみです。
- フィルタリングと検索は、選択したタスクのコメントにのみ適用されます。他の選択されていないタスクのコメントは影響を受けません。
