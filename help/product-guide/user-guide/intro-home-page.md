...
- **その他のアクション**：追加のオプションへのアクセスを提供します。 このボタンを選択すると、次のオプションを含むメニューが開きます。
   ...
   - **Workspace設定**: **Workspace設定** ダイアログが表示されます。 詳しくは、「Workspaceの設定 [ を参照してください ](../cs-install-guide/workspace-settings.md)。
     ...
     >[!NOTE]
     >
     > ホームページでは、「Workspace設定」オプションは、Cloud Services 設定でのみ使用できます。 オンプレミス設定では、ホームページで「その他のアクション」オプションを使用できません。 ただし、その他のオプション /設定に移動すると、エディターインターフェイスとマップコンソールを使用して、関連する設定にアクセスできます。
...
>[!NOTE]
>
> リポジトリは、2025.11.0 リリース以降のクラウドサービスの設定でのみ、左側のパネルで使用できます。 オンプレミス設定の場合、エディターのインターフェイスからリポジトリーに引き続きアクセスできます。

左側のパネルでは、概要、リポジトリ、マップコレクション、一括公開、パブリッシュキュー、ユーザーの環境設定機能にすばやくアクセスできます。 パネルを展開するには、インターフェイスの左下コーナーに配置されている **展開** アイコンを選択します。 展開したら、**折りたたむ** アイコンを使用してパネルを折りたたみます。
...
リポジトリは、フォルダーやファイルを簡単に検出するための一元的なハブとして機能します。 すべてのファイルとフォルダーの包括的な表形式のリスト表示と、それらのコンテキスト詳細を提供します。 この統合インターフェイスを通じて、堅牢なフィルタリングオプションでファイルをシームレスに参照して検索を実行し、効率と強化されたエクスペリエンスを確保できます。 [ リポジトリ ](./home-page-repository-view.md) の詳細情報。
...
- **外観**:「外観」タブには、アプリケーションのテーマおよびコンテンツ編集領域のソースビューを選択するオプションが用意されています。 このタブを使用して、次の設定を行います。
  ...
  ![](images/user_preference_editor_appearance.png){align="left"}
   ...
   - **エクスプローラーでファイルを常に検索**：エディターで編集している間にリポジトリ内のファイルの場所を表示するには、このオプションを選択します。
