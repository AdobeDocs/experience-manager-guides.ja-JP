...
- ネイティブ PDF |生成されるPDFで言語メタデータを WCAG 2.0 に準拠するように設定することはできません。 （12407）
...
- AEM Service Pack 16 または 17 のAEM サイト出力ページで検索すると、サイト検索で問題が発生する。

  回避策：

   1. `/libs/foundation/components/search/search.jsp` の `crx/de` というパスのファイルを開きます。
