---
title: AEM Guidesのレポート
description: AEMで利用できるすべてのレポートを確認して、コンテンツの品質向上に役立てることができます。
exl-id: f0d761ff-efea-452c-bb8e-865f34a182fc
feature: Report Generation
role: User, Admin
TQID: https://experienceleague.adobe.com/bW4L1eMZPdpVt8Z00HqvaOpwrbPB3A6Uxb2L7aXsVbU
product_v2:
  - id: fae5e35a-80c9-4b94-9352-1a060a6aab1d
  - id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2:
  - id: a3bd6397-2eb2-4908-a61c-226e26855dca
  - id: d90290ec-3e61-4ebd-8649-bcafe0836803
subfeature_v2:
  - id: cdab8659-8d50-4417-b6fd-762f347c13ee
  - id: d6596f3f-92a7-43ec-b444-237db6adad05
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
topic_v2:
  - id: a004cc84-67b9-4a33-a3a7-8ec7273ef4dc
  - id: aa2f3246-cb95-4b30-8899-fdf7d73550cc
  - id: ebde5b41-29c9-4f5e-9ef6-1197e85409e3
  - id: f5c2a4bb-71ca-4d7e-8efd-442250e6ba48
source-git-commit: 8ed5c9cb07c56b84b36ef56a55af8738989a6d3f
workflow-type: tm+mt
source-wordcount: 314
ht-degree: 1%

---

# AEM Guidesのレポート

組織設定では、ドキュメントをエンドユーザーが使用する前に、コンテンツの全体的な完全性を確認する必要があります。 マルチユーザー環境や大規模環境では、コンテンツを異なるコンテキストで再利用できるため、コンテンツやドキュメントを検証する必要があります。 構造化コンテンツのコンテキストでは、一般的に必要とされる特定のタイプのレポートがあります。


## セッションの概要

このセッションでは、ユーザーが利用できる標準レポートを紹介しました。 主な内容：
- ドキュメント（マップ）とトピックレポート
- メタデータレポートと一括メタデータ管理オプション
- コンテンツ再利用レポート
- コンテンツ読み込みの正確性
- Adobe AEM Assetsのレポート機能を活用することで


## セッション録画

>[!VIDEO](https://video.tv.adobe.com/v/3417529/guides--reporting-reporting?quality=12&learn=on)


## ハイライト

- **日付** - 2023年3月30日
- **参加者数** - 102
- **期間** - 60分
- **Audience** – お客様、パートナー、およびAdobeの従業員


## 重要ポイント

このセッションでは、次の項目について学習しました。
- CCMSにおけるレポートの重要性
- すぐに利用できるレポート
- コンテンツに関するレポートの例


## 関連リソース

- **[AEM Guides レポート ハンドブックをダウンロード](./assets/aem-guides-expert-session-reports-documentation.pdf)**&#x200B;して、セッションで示されたレポートの詳細を確認します。

- **機能ビデオ** - [Experience League](https://experienceleague.adobe.com/docs/experience-manager-guides-learn/videos/output-generation/working-with-reports.html?lang=en)

- **ユーザーガイド** - [&#x200B; コンテンツの作成](https://help.adobe.com/en_US/xml-documentation-for-adobe-experience-manager/index.html#t=DXML-master-map%2Freports-intro.html)

- **コミュニティフォーラム** - Experience Leagueでのセッション固有のコミュニティインタラクションについては、[AEM Guides フォーラム &#x200B;](https://experienceleaguecommunities.adobe.com/t5/experience-manager-guides/bd-p/xml-documentation-discussions)をご覧ください。

>[!NOTE]
>
> このセッションで説明する機能は、次のバージョンのAEM Guidesで利用できます。
> - オンプレミス：4.2以降
> - クラウドサービス：2302以降


## 次回のエキスパートセッション

- **トピック** - コンテンツとテンプレートの設定
- **スケジュール日：** - 2023年4月27日


## さらに詳しく

ご質問がある場合は、<techcomm@adobe.com>にメールをお送りください
