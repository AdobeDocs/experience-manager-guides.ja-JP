---
title: ネイティブ PDF パブリッシング – 例による学習（パート 1）
description: ネイティブのPDF パブリッシングエンジンを使用すると、更新しやすいCSS、HTML、JavaScriptを使用して、必要に応じて必要なセクションを操作しながら、ドキュメントのコンテンツレイアウトを制御できます。
...
このセッションでは、いくつかの例を参考に、PDFのネイティブ公開機能の活用方法について説明します。
...
- MathML、透かし、変更履歴などの基本機能を有効にする
- カスタムフォントの使用
- PDF出力でのDITA XMLのメタデータの使用
- PDF出力での複数列pdf出力の作成とページサイズの変更（印刷用）
- 特定のエレメントのレンダリング順序の変更（JavaScriptを使用）
...
> - オンプレミス 4.2以降
> - Cloud Services 2303以降

## 次回のエキスパートセッション

- **Topic** - PDFのネイティブ公開「Hands on lab - BYOS」 – インタラクティブ学習
- **スケジュール日：** - 2023年9月7日
