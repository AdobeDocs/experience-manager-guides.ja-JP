---
title: AEM Sitesの出力バリエーション
description: AEM GuidesからAEM Sites出力のさまざまなバリエーションを生成する方法について説明します
exl-id: 47881e72-b289-4bf0-8824-55825f1502b2
TQID: https://experienceleague.adobe.com/DykSVQyGxlsZ6NylX9gNhb0h7xgsPBygksCLqbeK8Og
product_v2:
  - id: fae5e35a-80c9-4b94-9352-1a060a6aab1d
  - id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2:
  - id: a3bd6397-2eb2-4908-a61c-226e26855dca
  - id: cb8c6a2a-3c38-4e40-867c-756f8c36bb0e
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
topic_v2:
  - id: a004cc84-67b9-4a33-a3a7-8ec7273ef4dc
  - id: e9001ce2-5245-4a8e-8601-dd958009072f
source-git-commit: 8ed5c9cb07c56b84b36ef56a55af8738989a6d3f
workflow-type: tm+mt
source-wordcount: 271
ht-degree: 6%

---

# セッションの概要

web エクスペリエンスのレイアウトが明確に定義されていれば、アプリケーションのブランディングにも一貫性があります
デジタル全体の統一性や一貫性を維持しながら
エクスペリエンス/ページです。
これらはすべて、Adobe experience manager Guidesが提供するAEMサイト出力の様々なバリエーションで実現できます。
このセッションでは、AEM Sites出力のさまざまなバリエーションを生成する方法について説明します。

## セッション録画

>[!VIDEO](https://video.tv.adobe.com/v/3430649/)

## 重要ポイント

このセッションでは、次の項目について説明します。

- AEMのさまざまなテンプレートタイプのサービス
- テンプレートや公開メカニズムにもとづいたバリエーションの作成方法
- 各テンプレート/メカニズムの使い方の違い
- 既存のAdobe Experience Manager Sites テンプレートを使用して、Adobe AEM Sitesの出力を構築

## ハイライト

- **日付** - 2024年6月27日
- **参加者の数** - 120
- **期間** - 60分
- **Audience** – お客様、パートナー、およびAdobeの従業員

## 関連リソース


- [AEM サイト出力の公開](https://experienceleague.adobe.com/en/docs/experience-manager-guides/using/user-guide/output-gen/output-presets-aemg/generate-output-aem-site#:~:text=To%20open%20output%20presets%20for,configurations%2C%20and%20then%20click%20Save)

- [KB Output パブリッシング](https://experienceleague.adobe.com/en/docs/experience-manager-guides/using/user-guide/output-gen/output-presets-aemg/generate-output-knowledge-base)

- [AEM テンプレート](https://experienceleague.adobe.com/ja/docs/experience-manager-65/content/implementing/developing/platform/templates/templates)

- **コミュニティフォーラム** - Experience Leagueでのセッション固有のコミュニティインタラクションについては、[AEM Guides フォーラム &#x200B;](https://experienceleaguecommunities.adobe.com/t5/experience-manager-guides/bd-p/xml-documentation-discussions)をご覧ください。

>[!NOTE]
>
> このセッションで紹介する機能は、AEM GuidesのCloud リリースバージョン 2406に記載されています。

## さらに詳しく

ご質問がある場合は、<techcomm@adobe.com>にメールをお送りください
