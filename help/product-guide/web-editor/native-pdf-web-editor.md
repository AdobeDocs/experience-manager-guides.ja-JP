...
>[!NOTE]
>
>- DITA関連テーブル （`<reltable>`）で定義された関連リンクは、デフォルトではネイティブ PDF出力に含まれません。 このフィールドを使用して`-Dargs.rellinks=nofamily` DITA-OT引数を渡し、そのような関連リンクを出力に含めます。
>
...
プリンターマークの割り当て、ページボックスの定義、PDF出力のカラーおよびICC プロファイルオプションの設定を行うには、プリントの作成設定を行います。 「印刷」タブは、次の3つのセクションに分かれています。**プリンターマーク**、**ページボックス**、**カラーとICC**。

## プリンターマーク

印刷用にドキュメントを準備する際には、プリンターマークがページの境界に追加され、印刷中の適切な整列、トリミング、カラー選択に役立ちます。 プリンターマークを選択すると、ページの境界が拡大され、プリント後に拡大領域がトリミングされます。

PDF出力のプリンターマークを設定するには、次の設定を使用します。

| 設定 | 説明 |
| --- | --- |
| **すべてのプリンターのマーク** | すべてのプリンターマーク設定を有効または無効にする場合に選択します：**トリムマーク**、**裁ち落としマーク**、**登録マーク**、および&#x200B;**カラーバー**&#x200B;を一緒に選択します。 **すべてのプリンタのマーク**&#x200B;が選択されている間に個別のマークを有効または無効にした場合、この設定は自動的にオフになります。 この設定を選択しない場合は、個々のマークを有効または無効にできます。 |
| **トンボのトリミング** | プリント後に用紙をトリミングする場所を示すために、トリム領域の各隅にマークを配置します。 |
| **裁ち落としマーク** | 選択すると、裁ち落としボックスの各隅にマークが配置され、拡張された画像のトリム領域が示されます。 |
| **登録記号** | カラー文書内の様々な色分解を整列するために、切り抜き領域の外側にマークを配置する場合に選択します。 |
| **カラーバー** | 選択すると、トリム領域の外側に色のストリップが追加され、色の一貫性が維持され、印刷時のインク濃度が調整されます。 |
| **線幅** | 選択したプリンターマークの描画に使用する線の幅を指定します。 値は0.25 ptのステップで増加します。 |
| **線の色** | 選択したプリンターマークの描画に使用する線の色を指定します。 |

## ページボックス

このセクションを使用して、PDF出力の全体的なページサイズと裁ち落とし領域を定義します。

ページボックスを設定するには、次の設定を使用します。

| 設定 | 説明 |
| --- | --- |
| **メディアボックスサイズ** | プリンターマークが占める拡大領域を含め、全体的なページサイズを指定します。 ドロップダウンリストを使用して、PDF出力のページサイズを選択するか、カスタムサイズを作成します。 |
| **裁ち落としボックスの幅** | 裁ち落としボックスの幅を指定します。これは、裁ち落としマークを含むトリムボックスの外側に広がる領域と、トリムラインを超えて広がる画像です。 |

## カラーとICC

このセクションを使用して、プリント用のカラースペースを選択し、オプションでICC プロファイルを使用してデバイス間のカラー精度を管理します。 CMYK カラースペースを使用する場合、カラーマネジメントが必要なPDF準拠にはICC カラープロファイルが必要です。

>[!NOTE]
>
> ICC カラープロファイルの処理を改善するには、[Native PDF engine v2](../native-pdf/new-pdf-engine.md)を使用していることを確認してください。これには、Native PDF engine v1で発生する既知のICC カラープロファイルの問題に対する修正が含まれています。

次の設定を使用して、カラースペースとICC プロファイルを設定します。

| 設定 | 説明 |
| --- | --- |
| **カラースペース** | PDF ドキュメントの&#x200B;**RGB**&#x200B;と&#x200B;**CMYK**&#x200B;のカラースペースを選択します。 デジタルディスプレイ用のPDFの場合は&#x200B;**RGB**&#x200B;を選択し、物理的な印刷用のPDFの場合は&#x200B;**CMYK**&#x200B;を選択します。 |
| **色を変換** | ドキュメントで定義されているカラーを、上記で選択したカラースペースに変換する場合に有効にします。 このオプションは、RGBおよびCMYK カラースペースに対してデフォルトで有効になっています。 有効にすると、**レンダリングインテント** フィールドも使用できるようになります。 |
| **インテントのレンダリング** | 「**カラーを変換**」オプションが有効になっている場合にのみ使用できます。 ターゲットのカラースペースにカラーを変換する際に使用するレンダリングインテントを選択します。<br>- **Default**：デフォルトのレンダリング動作を使用します。<br>- **Perceptual**：カラーを調整して、全体的な視覚的外観を維持します。<br>- **相対的な色域**：可能な限り、ターゲットのカラースペースに基づいてカラーを調整します。<br> **メモ**：この機能をAdobe Experience Manager Guides オンプレミス設定で使用する場合は、基になるNative PDF エンジンでJava 20以降が使用可能であることを確認して、指定されたレンダリングインテントを適用します。 |
| **識別子名** | 出力識別子の名前を入力します。 |
| **プロファイルを参照** | AEM サーバー上のICC プロファイル ファイル （`.icc`または`.icm`）を参照します。 このフィールドは、**プロファイルにURLを使用**&#x200B;が有効になっている場合は無効になります。 |
| **プロファイルにURLを使用** | ローカルファイルの代わりにオンラインでホストされているICC プロファイルを使用するには、このオプションを有効にします。 |
| プロファイル **の** URL | **プロファイルにURLを使用**&#x200B;が有効になっている場合にのみ使用できます。 ICC プロファイルがホストされているURLを入力します。 |
...
| **透かしを表示** | 出力に透かしを重ねるには、このオプションを選択します。 テキストボックスに新しいテキスト文字列を入力し、文字の大文字と小文字を変更できます。 <br><br>静的なテキスト変数または言語変数を使用して、ローカライズされたバージョンの透かしを公開します。  選択した言語に応じて、ローカライズされた値がPDF出力で自動的に選択されます。 例えば、英語では「Publisher」を、フランス語では「Auteure」を透かしとして印刷できます。 <br> 形式：`${lng:<variable name>}`。 例えば、`publisher-label`が言語変数である`$ {lng:publisher-label}`とします。<br> カーソルを合わせる オプションの近くにある<img src="./assets/info-details.svg" alt= "情報アイコン" width="25">をクリックすると、その詳細が表示されます。 |
