...
* **メタデータ名と値を指定**
   ...
   1. メタデータの値を入力し、「+」アイコンを選択します。PDFのメタデータのリストにメタデータが追加されます。

変数を使用してメタデータ値を定義することもできます。  DITA マップファイルまたはブックマップファイルに定義されているメタデータを変数として使用できます。 メタデータは、DITA マップまたはブックマップファイルの`/jcr:content/metadata` ノードにあります。変数を使用する場合、その値はメタデータプロパティから選択されます。
