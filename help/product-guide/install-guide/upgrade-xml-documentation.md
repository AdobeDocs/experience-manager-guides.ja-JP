...
Experience Manager Guidesの現在のバージョンをバージョン 5.0.0 にアップグレードするには、次の手順を実行します。


- バージョン 4.6.x、4.6、または 4.4 を使用している場合は、バージョン 5.0.0 に直接アップグレードできます。
...
Experience Managerガイドのアップグレードプロセス開始前に、以下を確認する必要があります。

1. レビュー用に開いているトピックのレビューコメントをインポートしました。
1. すべてのアクティブなレビューを閉じました。
1. すべての翻訳タスクをクローズしました。
...
| --- | --- |
| リクエストタイプ | **GET** 管理者として AEM インスタンスにログインしているWeb ブラウザーを使用できます。 |
| 予想される応答 | - 必要なすべてのノードを移動できる場合は、合格したチェックを取得します。 <br>- ターゲット場所にノードが存在する場合は、関連するエラーが表示されます。 リポジトリー \(ノード /var/dxml\ を削除) をクリーンアップし、アップグレード パッケージを再インストールしてから、このエンドポイントを再度トリガーします。 <br>**メモ：** これは一般的なエラーではありません。3.x Experience Manager Guidesでは、以前はターゲットの場所が使用されていないからです。 <br> -   このスクリプトが成功しない場合は、続行してカスタマーサクセスチームに報告しないでください。 |
...
Experience Manager ガイドをインストールした後、新しくインストールしたバージョンから適用可能なさまざまな設定をセットアップにマージできます。

>[!NOTE]
>
> dam-update-アセット モデルはカスタマイズできます。 したがって、カスタマイズが行われた場合は、カスタマイズとExperience Managerガイドをモデルの作業コピーに同期する必要があります。

1. **DAM アセットの更新ワークフロー\(Post-processing changes\):**
...
>[!NOTE]
>
>投稿処理とインデックス作成には数時間かかる場合があります。 アップグレードプロセスは、ピーク時を避けて開始することをお勧めします。

**&#x200B;**&#x200B;前提条件&#x200B;**&#x200B;**

Experience Manager ガイド 4.2.1 のアップグレードプロセス開始前に、以下が揃っていることを確認します。

1. Experience Manager ガイド バージョン 4.1、4.1.x または 4.2 にアップグレードしました。
1. すべての翻訳タスクを終了しました。
1. `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript` のクラスのログレベルを **INFO** に変更し、これらのログを新しいログファイル（例：`logs/translation_upgrade.log.`）に追加しました
...
1. アップグレードが完了したら、カスタマイズやオーバーレイが検証され、新しいアプリケーションコードに一致するように更新されます。 以下に例をいくつか示します。
   ...
   - elementmapping.xml
   - ui\_config.json\(フォルダープロファイルで設定されている可能性があります\)
   - 改正 `com.adobe.fmdita.config.ConfigManager`
   - カスタムコードのいずれかが古いパスを使用していたかどうかを確認します\( [移行マッピング](#id2244LE040XA) セクションで説明したように)-カスタマイズも期待どおりに機能するように、新しいパスに更新する必要があります。
...
**親トピック:**&#x200B;[&#x200B;無償体験版で試してみるおよびインストール](download-install.md)


## バージョン 4.6.0 へのアップグレード専用

>[!TIP]
>
> バージョン 4.6.0、4.6.0 サービス Pack 1、または 4.6.0 サービス Pack 3 の上に 4.6.0 サービス Pack 4 をインストールすることをお勧めします。 4.6.0 サービス Pack 4 リリースのアップグレード プロセスは、4.6.0 リリースと同じ手順に従います。
...
1. Experience Manager Guides バージョン 4.3.1、4.3.0、または 4.2.1(ホットフィックス 4.2.1.3)にアップグレードし、それぞれのインストール手順を完了。
1. (オプション)すべての翻訳タスクをクローズしました。
1. `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`クラスのログレベルを **INFO** に変更し、これらのログを新しいログファイル (`logs/translation_upgrade.log` など) に追加します。


## バージョン 4.6.0 をインストール

1. [Adobe Systemsソフトウェア配布ポータル](https://experience.adobe.com/#/downloads/content/software-distribution/ja/aem.html)からの無償体験版で試してみる 4.6.0バージョンパッケージ。
1. バージョン 4.6.0 パッケージをインストールします。
1. 変換マップのアップグレード ジョブ開始トリガーをヒットするように選択できます。 詳しくは、 [サーブレット経由でスクリプトのトリガーを有効にする](#enable-trigger-of-script-via-a-servlet)を参照してください。
...
1. パッケージのインストールが完了したら、ログに次のメッセージが表示されるまで待ちます。
   ...
   次の ERROR プレフィックスのいずれかに遭遇した場合は、顧客成功チームに報告してください。

   - 投稿 デプロイメントセットアップスクリプトでのエラー
   - 翻訳マップの移行中の例外
   - プロパティの翻訳マップを v1 から v2 にポートできません
...
1. damAssetLucene にカスタマイズを追加した場合は、必要に応じて再度適用します。 これらの変更を行った後、reindex を true に設定します。 これにより、カスタマイズした既存のすべてのノードのインデックスが再作成されます。 完了すると、再インデックスフラグは再び false に設定されます。 システム内のアセット数によっては、数時間かかる場合があります。

## Experience Managerガイドのインデックスを再作成する手順

1. 開く `crx/de` 、インデックスパスに移動します。  `/oak:index/guidesAssetProperties`
...
コードベースに別のカスタム Sling リライターがある場合は、Experience Manager Guides Sling Rewriter が `'order'` 50 を使用するように、50 より大きい`'order'`値を使用します。これをオーバーライドするには、値 >50 が必要です。 詳細については、「 [出力パイプラインの書き換え](https://sling.apache.org/documentation/bundles/output-rewriting-pipelines-org-apache-sling-rewriter.html)」表示を参照してください。

このアップグレード中に、 `'order'` 値が 1000 から 50 に変更されるため、既存のカスタム リライター (存在する場合) を `'fmdita-rewriter'` とマージする必要があります。
...
>[!TIP]
>
> バージョン 5.0.0 へのアップグレードは、Experience Manager Guidesの現在のバージョンによって異なります。 バージョン 4.6.3、4.6.1、4.6.0、または 4.4 を使用している場合は、バージョン 5.0.0 に直接アップグレードできます。
...
1. damAssetLucene でカスタマイズを追加した場合は、再度適用する必要があります。 これらの変更を行ったら、reindex を true に設定します。 これにより、カスタマイズした既存のすべてのノードのインデックスが再作成されます。 完了すると、再インデックスフラグは再び false に設定されます。 システム内のアセット数によっては、数時間かかる場合があります。
