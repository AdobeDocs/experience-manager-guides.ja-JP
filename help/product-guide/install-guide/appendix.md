...
The `paraRule` 要素は必須です。 すべての段落スタイルのマッピングルールを指定します。 InDesignドキュメントでは、スタイルのない段落にも名前が付けられ、すべてのテキストが段落スタイルのサブ構造内に含まれます `[No paragraph style]`. 角括弧は、組み込みのInDesignスタイル名を示します。
...
**`elementRule`要素** The `elementRule` 要素は必須です。 これらは、すべてのソース要素のマッピングルールです。 InDesignドキュメントには非構造化スタイル要素が含まれていますが、構造化コンテンツの場合は「***ハイブリッドモード***「 」の処理が有効になっています。
