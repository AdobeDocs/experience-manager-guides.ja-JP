...
デフォルトでは、AEMはDITA コンテンツを認識しないため、リポジトリ内のDITA コンテンツを検索する仕組みは提供されません。 AEM Guidesでは、DITA コンテンツ検索機能をAEM リポジトリに追加できます。
...
1. ユーザーとグループの権限ページにアクセスします。 ページにアクセスするためのデフォルトのURLは次のとおりです。
...
configMgrでメタデータ抽出オプションを有効にするには、次の手順を実行します。
