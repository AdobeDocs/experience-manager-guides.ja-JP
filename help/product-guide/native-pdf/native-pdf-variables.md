...
1. **名前** 列に変数名を入力し、**値** 列にその値を入力します。
   >[!TIP]
   >
   >任意のHTML内容を変数値として使用して、変数値を特定の形式で表示できます。 例えば、変数値に `<b>` タグを追加して、値 **1}Experience Manager Guides} を太字で表示できます。**&#x200B;リポジトリから画像を値として追加することもできます。
...
例えば、変数セット *Adobe-set1} および* Adobe-set2 *に次の値を設* できます。
