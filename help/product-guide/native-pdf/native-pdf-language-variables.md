...
AEM Guidesには、言語変数を使用する機能が用意されています。 言語変数を使用して、PDF出力にローカライズされた文字列を定義したり、出力テンプレートに静的テキストをローカライズしたりできます。 CSS スタイルを使用して、CSS から取得する文字列をローカライズすることができます。

## PDF出力での言語変数の使用

PDF変数を使用して、標準提供のラベルのローカライズ版を定義できます（メモ、注意、警告、静的テキストなど）。 変数名はすべての言語で同じですが、様々な言語で異なる値を持つことができます。 これらの変数の値を 1 つ以上のPDFで更新すると、ローカライズされた値が言語の出力で自動的に選択されます。

例えば、PDF出力にラベル `Note` を表示するには、次の方法があります。
...
>
> UI の言語で値を定義していない場合は、英語（`en_us`）の値が検索されます。定義していない場合は、英語（`en`）の値が選択され、PDF出力に表示されます。
...
1. **言語変数** を選択します。 左パネルで <img src="./assets/language-variables.svg" width="25"> をクリックします。
...
1. **言語** ドロップダウンから、変数を編集する言語を選択します。
   ...
   >[!NOTE]
   >
   > 目的の言語が表示されない場合は、「言語変数設定 **から目的の言語を有効に** ます。 設定を選択 **言語変数設定** ダイアログを開く <img src="./assets/settings-icon.svg" width="25"> 要があります。

1. **名前** 列に変数名を、**値** 列に値を入力します。

   >[!NOTE]
   >
   >任意のHTMLコンテンツを変数値として使用し、変数値を特定の形式で表示できます。 例えば、変数値にタグ `<b>` 追加して、公開者を太字で表示できます。
...
また、アプリケーション変数の値を編集することもできます。 後で、アプリケーション変数を元の値に戻すことができます。 **変数を復帰** <img src="./assets/application-variable-revert.svg" width="25"> は、値が変更されたアプリケーション変数に対して表示されます。
...
HTML変数に割り当てた値の他に、言語タグを使用して変数値を特定の形式で表示することもできます。 例えば、`publisher-label` の値を太字で表示することができます。
...
- また、値にローカライズされた画像を追加することもできます。 例えば、チャプター番号PDFに画像アイコンを追加すると、そのアイコンのローカライズされた画像を言語の出力時に取得できます。
...
次のスクリーンショットは、ドイツ語出力および日本語PDF出力にローカライズされた文字列を示しています。
