---
title: ネイティブのPDF公開機能 |共通のコンテンツスタイルの使用
...
スタイルエディターは、ユーザーインターフェイスの背後で CSS コードの複雑さをすべて非表示にするWYSIWYG エディターです。 スタイルエディターを使用すると、選択した要素のスタイルを簡単にすばやくカスタマイズできます。 スタイルは、次の見出しに分類されます。
...
PDFのネイティブパブリッシング機能には、最も一般的なスタイルの基本スタイル定義が含まれているので、誤ってスタイルを削除した場合でも、コンテンツにはデフォルトスタイルが適用されます。 例えば、スタイルシートに h2 スタイルのスタイル定義がない場合、Native PDFの公開機能によって h2 コンテンツにベーススタイルが適用されます。
...
1. *スタイルを追加* ダイアログで、**タグ** 名を `h2` のままにし、`chapter` クラス **名フィールドに** を入力します。
1. 「**完了**」をクリックします。
...
1. 次のプロパティを設定します。
   * **スタイル**: ロケール固有または一般的な様々な番号付けスタイルから選択します。 アラビア語 – インド語、デバナガリ語、グルジア語、10 進数、下Alphaなどのスタイルを選択できます。 現在の例では、「`upper-alpha`」を選択します。
...
ドロップキャップ（またはドロップ資本）スタイルは、雑誌や文書類で使用され、段落またはセクションの最初の文字に特別なスタイルが設定されています。 PDFのネイティブ公開機能を使用しても同じ効果を得ることができます。
...
1. スタイルを追加ダイアログで、**タグ** の名前を span のままにし、`BoldItalic` クラス **の名前フィールドに** と入力します。
...
1. *スタイルを追加* ダイアログで、**タグ** 名を `table` のままにし、`double-border` クラス **名フィールドに** を入力します。

1. 「**完了**」をクリックします。

   `table.double-border` という名前の新しいテーブルスタイルが作成され、テーブルスタイル リストに追加されます。

1. `table.double-border` テーブルスタイル **リストから** を選択し、次のプロパティを設定します。
   ...
   * **境界線の間隔**：この設定は、「境界線を折りたたむ」が「区切る」に設定されている場合にのみ使用できます。 この設定を使用すると、セルの罫線の間の垂直方向と水平方向の間隔を指定できます。
     ...
     >[!NOTE]
     >
     >次の設定は、[ 書式の適用先 **が [ テーブル全体** に設定されている場合に [**セル**] セクションで使用 **きます**。
...
1. *スタイルを追加* ダイアログで、**タグ** 名を *blank* のままにし、`wintitle` クラス **名フィールドに** を入力します。
...
ネイティブのPDF出力を公開する際に、すべてのスタイルが最終的なPDFに結合されるので、CSS 内の各テンプレートに一意のスタイルを割り当てることが重要です。
...
カスタム CSS スタイルを定義すると、ネイティブのPDF出力の生成時に、それらが最初に優先されます。
...
`<note>` 要素は、type 属性に対応するクラス prefix-content を持つ追加の `<span>` を生成します。 この CSS ルールは、type 属性を持つ `.prefix-content` 要素内の `<note>` クラスをターゲットにし、必要に応じてプレフィックスのコンテンツのスタイル設定または操作を行えるようにします。
