...
1. *スタイルを追加* ダイアログで、**タグ**&#x200B;の名前を`h2`のままにし、**クラス**&#x200B;の名前フィールドに`chapter`と入力します。
1. 「**完了**」をクリックします。
...
1. スタイルを追加ダイアログで、**タグ**&#x200B;の名前をスパンとして保持し、**クラス**&#x200B;の名前フィールドに`BoldItalic`と入力します。
...
1. *スタイルを追加* ダイアログで、**タグ**&#x200B;の名前を`table`のままにし、**クラス**&#x200B;の名前フィールドに`double-border`と入力します。

1. 「**完了**」をクリックします。

   `table.double-border`という名前の新しいテーブル スタイルが作成され、テーブル スタイル リストの下に追加されます。

1. **表スタイル** リストから`table.double-border`を選択し、次のプロパティを設定します。
...
1. *スタイルを追加* ダイアログで、**タグ**&#x200B;の名前を&#x200B;*空白*&#x200B;のままにし、**クラス**&#x200B;の名前フィールドに`wintitle`と入力します。
...
`<note>`要素は、type属性に対応するクラスのprefix-contentを持つ追加の`<span>`を生成します。 このCSS ルールは、`<note>`要素の`.prefix-content` クラスをtype属性でターゲットとし、必要に応じてプレフィックスコンテンツをスタイル設定または操作できるようにします。
